...
from app.models import Token, SnsType, UserRegister, UserToken
import os
...
@router.post("/v1/register/{sns_type}", status_code=200, response_model=Token)
async def register(sns_type: SnsType, reg_info: UserRegister, session: Session = Depends(db.session)):
    ...
    if sns_type == SnsType.email:
        ...
        return JSONResponse(status_code=200, content=dict(token = token, msg="ok"))
    return JSONResponse(status_code=400, content=dict(msg="NOT_SUPPORTED"))


@router.post("/v1/login/{sns_type}", status_code=200)
...
def create_access_token(*, data: dict = None, expires_delta: int = None):
    ...
    encoded_jwt = jwt.encode(to_encode, JWT_SECRETE, algorithm=JWT_ALGORITHM)
